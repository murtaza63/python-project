...
class Heap(Generic[Element]):
    ...
    # NOTE: This is a new method just in this version of the file
    def sift_down_from_index(self, index: int) -> None:
        self.sift_down_from_index_upto_size(index, len(self.elements))

    # NOTE: This is a new method just in this version of the file
    def sift_down_from_index_upto_size(self, index: int, size: int) -> None:
        parent = index
        while True:
            left = self.left_child_index_of_parent_at_index(parent)
            right = self.right_child_index_of_parent_at_index(parent)
            candidate = parent
            if left < size and self.sort(self.elements[left], self.elements[candidate]):
                candidate = left
            if right < size and self.sort(self.elements[right], self.elements[candidate]):
                candidate = right
            if candidate == parent:
                return
